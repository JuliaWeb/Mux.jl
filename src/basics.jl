...
An HTTP response with status, headers, and body corresponding to the keys
`:status`, `:headers`, and `:body` in `d`.

If any key is absent, it is set to a default value (200 or empty).
"""
Response(d::AbstractDict) =
...
An HTTP response that uses the "text/html" representation of `o`, as generated by `stringmime`.
"""
